...
.factory("SessionService", function ($window, $state, config) {
    ...
    function GetSiteURL() {
        var url = config.App_Link;
        var str = config.App_Link;
    ...
    function GetPaymentSiteURL() {
        var url = config.Payment_Link;
        var str = config.Payment_Link;
        if (str.search("localhost") > -1) {
            url = url + "build";
        }
        return url;
    ...
    return {
        ...
        GetPaymentSiteURL: GetPaymentSiteURL,
