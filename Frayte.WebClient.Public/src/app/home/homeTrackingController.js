 ...
 * Controller
 */
angular.module('ngApp.home').controller('HomeTrackingController', function (AppSpinner, $uibModal, $scope, $translate, $location, $anchorScroll, $state, $stateParams, config, $filter, HomeService, SessionService, $log, DateFormatChange) {
    ...
    $scope.GetTrackingDetail = function (carriertype, trackingNumber) {
        if (carriertype === "UKEUShipment") {
            ...
            HomeService.ParcelHubTracking(carriertype, trackingNumber.replace(/\s/g, '')).then(function (response) {
                ...
                if (response.data != null && !response.data.Status) {
                    ...
                    var json2 = response.data.Tracking;
                    if (json2.length > 0) {
                        for (var j = 0 ; j < json2.length; j++) {
                            json2[j].TrackingDetails = $scope.TrackingDetailJson(json2[j]);
                        }
                        $scope.trackingInfos = json2;
                    }
        ...
        else if (carriertype === "" || carriertype === null || carriertype === undefined) {
            HomeService.GettrackingEcomm(carriertype, trackingNumber.replace(/\s/g, '')).then(function (response) {
                AppSpinner.hideSpinnerTemplate();
                ...
                var json = [];
                json[0] = response.data;
                if (json.length > 0) {
                    for (var i = 0 ; i < json.length; i++) {
                        json[i].TrackingDetails = $scope.TrackingDetailJson(json[i]);
                        json[i].IsHeaderShow = true;

                        //json[i].TrackingDetails.Detail[0].IsCollapsed = false;
                    }
                    $scope.trackingInfos = json;
            ...
            HomeService.GettrackingDetail(carriertype, trackingNumber.replace(/\s/g, '')).then(function (response) {
                ...
                if (response.data != null && !response.data.Status) {
                    ...
                    var json1 = response.data.Tracking;
                    if (json1.length > 0) {
                        for (var i = 0 ; i < json1.length; i++) {
                            json1[i].TrackingDetails = $scope.TrackingDetailJson(json1[i]);
                        }
                        $scope.trackingInfos = json1;
                    ...
                    if (response.data.Tracking[0].CarriertrackingId !== null && response.data.Tracking[0].CarriertrackingId !== undefined && response.data.Tracking[0].CarriertrackingId !== '') {
                        if (response.data !== null && response.data.Status && response.data.Tracking !== null && response.data.Tracking[0].TrackingDetails !== null && response.data.Tracking[0].TrackingDetails.length) {
                            ...
                            var trackingErrorMessage = $uibModal.open({
                                templateUrl: 'home/trackingErrorMessage.tpl.html',
                                windowClass: '',
                                size: 'lg'
                            });
                            trackingErrorMessage.result.then(function () {
                                $state.go('home.welcome');
                            }, function () {
                                $state.go('home.welcome');
                            });
    ...
    $scope.printToCart = function (divName) {
        ...
        if (navigator.userAgent.toLowerCase().indexOf('chrome') > -1) {
            ...
            if (config.BUILD_URL.includes("build")) {
                popupWin.document.write('<!DOCTYPE html><html><head>' +
                  '<link rel="stylesheet" type="text/css" href= ' + $scope.BootstrapURL + ' />' +
                '<link rel="stylesheet" type="text/css" href= ' + $scope.FontAwesomeURL + ' />' +
                '<link rel="stylesheet" type="text/css" href= ' + $scope.FrateAppCssURL + ' />' +
    ...
    function init() {
        ...
        if (config.BUILD_URL.includes("build")) {
            $scope.FontAwesomeURL = config.Public_Link + "build/vendor/font-awesome/font-awesome.css";
            $scope.BootstrapURL = config.Public_Link + "build/vendor/bootstrap/bootstrap.css";
            $scope.FrateAppCssURL = config.Public_Link + "build/assets/Frayte.Public-1.0.0.css";
        }
        else {
            HomeService.GetDateFromFile().then(function (res) {
                var result = res.data;
                if (result.data !== null || result.data !== "" || result.data !== undefined) {
                    $scope.FrateAppCssURL = config.Public_Link + "assets/Frayte.Public-1.0.0" + "-" + result + ".css";
                }
            });
        }
