angular.module('ngApp.thirdPartyMatrix').controller('ThirdPartyMatrixController', function ($scope, AppSpinner,toaster, ZoneBaseRateCardService, $state, $translate, ZonePostCodeService, ThirdPartyMatrixService) {
    ...
    var getThirdPartyZones = function () {
        ...
        ThirdPartyMatrixService.GetZones(OperationZoneId, LogisticType, CourierCompany, RateType, ModuleType).then(function (response) {
            if (response.data !== null && response.data.length > 0) {
                $scope.ThirdPartyZones = response.data;
                getScreenInitials();
            }
            else {
                AppSpinner.hideSpinnerTemplate();
    ...
    function init() {
        ...
        $scope.spinnerMessage = 'Loading 3rd Party Matrix';
        $scope.ThirdPartyList = [];
        AppSpinner.showSpinnerTemplate($scope.spinnerMessage, $scope.Template);
        ThirdPartyMatrixService.GetOperationZone().then(function (response) {
