angular.module('ngApp.eCommerce').controller('eCommerceBookingController', function ($uibModalStack, UtilityService, HSCodeService, $window, eCommerceBookingService, Upload, $rootScope, AppSpinner, $sce, TopCountryService, $location, $anchorScroll, TopCurrencyService, $scope, config, $filter, $state, ModalService, $translate, $uibModal, $timeout, toaster, $stateParams, DirectBookingService, SessionService) {
    ...
    $scope.selectHSCode = function (Package) {
        ...
        eCommerceBookingService.SetShipmentHSCode(Package.DirectShipmentDetailDraftId, Package.HSCode).then(function (response) {
    ...
    $scope.onSelect = function ($item, $model, $label, $event, Package) {
        debugger;
        eCommerceBookingService.SetShipmentHSCode(Package.DirectShipmentDetailDraftId, $item.HSCode).then(function (response) {
            if (response.data.Status) {
    ...
    $scope.getHSCodes = function (query, serachType, Package) {
        ...
        if ($scope.eCommerceBooking.ShipTo.Country !== undefined &&
            $scope.eCommerceBooking.ShipTo.Country !== null &&
             $scope.eCommerceBooking.ShipTo.Country.CountryId > 0) {
            return HSCodeService.GetHSCodes(query, $scope.eCommerceBooking.ShipTo.Country.CountryId, serachType).then(function (response) {
            ...
            toaster.pop({
                ...
                body: "Please select Delivery Address country first and try again.",
                showCloseButton: true
            });
            ...
            if ($scope.BookingType === "Draft" || (isValid && $scope.isCollectionTimeValid)) {
                ...
                if (!flage) {
                    toaster.pop({
                        type: 'warning',
                        title: $scope.TitleFrayteValidation,
                        body: "Please set Pieces HsCode properly.",
                        showCloseButton: true
                    });
                    return;
                }
                ...
                if ($scope.BookingType === "Draft") {
                    ...
                    AppSpinner.showSpinnerTemplate('Saving Infomation', $scope.Template);
                }
                else {
                ...
                eCommerceBookingService.SaveeCommerceBooking(eCommerceBooking).then(function (response) {
                            ...
                            if (response.data.ShipmentStatusId !== $scope.ShipmentStatus.Draft) {
                                ...
                                modalInstance.result.then(function () {
                                    ...
                                    if ($scope.RoleId === 1) {
                                        ...
                                        $state.go('customer.direct-shipments', { moduleType: "eCb" }, { reload: true });
                                    }
    ...
    $scope.directBookingAddEdit = function (row) {
        ...
        modalInstance.result.then(function (addressBooks) {
            if (addressBooks !== undefined && addressBooks !== null && addressBooks.length > 0) {
                $scope.eCommerceBooking.ShipFrom = addressBooks[0].entity;
                $scope.eCommerceBooking.ShipFrom.Country = addressBooks[0].entity.Country;
    ...
    $scope.directBookingAddEditShipToAddress = function () {
        ...
        modalInstance.result.then(function (addressBooks) {
            if (addressBooks !== undefined && addressBooks !== null && addressBooks.length > 0) {
                $scope.eCommerceBooking.ShipTo = addressBooks[0].entity;
    ...
    function init() {
        ...
        DirectBookingService.GetInitials($scope.customerId).then(function (response) {
            ...
            if (eCommerceShipmentId > 0) {
                ...
                eCommerceBookingService.GeteCommerceBookingDetailDraft(eCommerceShipmentId, CallingType).then(function (response) {
