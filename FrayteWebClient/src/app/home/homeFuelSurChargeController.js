...
angular.module('ngApp.home').controller('HomeFuelSurChargeController', function ($scope, $location, $anchorScroll, $state, $stateParams, config, $filter, CountryService, CourierService, HomeService, SessionService, $uibModal, $log, toaster, FuelService, $translate) {
    var setModalOptions = function () {
        $translate(['Address', 'Book', 'DeleteHeader', 'DeleteBody', 'Tradelane', 'SuccessfullySavedInformation', 'FrayteInformation',
            ...
            'detail', 'TrackingDetails_Validation', 'records', 'Success']).then(function (translations) {
    ...
    function opezone() {
        FuelService.CurrentOperationZone().then(function (response) {
    ...
    function getfuel() {
        ...
        FuelService.GetThreeFuelSurCharge($scope.OperationZone.OperationZoneId, new Date()).then(function (response) {
