 ...
 * Controller
// */
angular.module('ngApp.termandcondition').controller('TermAndConditionController', function ($scope, $state, $translate, uiGridConstants, TermAndConditionService, config, $filter, LogonService, SessionService, $uibModal, toaster) {

    //Set Multilingual for Modal Popup
    var setModalOptions = function () {
        $translate(['FrayteError', 'ErrorGetting', 'TermsAndCondition', 'records']).then(function (translations) {
    ...
    $scope.SetGridOptions = function () {
        $scope.gridOptions = {
            ...
            columnDefs: [
              { name: 'Detail', displayName: 'TermsAndCondition', headerCellFilter: 'translate', width: '75%', cellTemplate: temp },
              { name: 'CreatedOn', displayName: 'Created_On', headerCellFilter: 'translate', width: '15%', cellFilter: 'dateFilter:this' },
    ...
    var getAllTermAndCondition = function () {
        if ($scope.OperationZone !== undefined && $scope.OperationZone !== null) {
            TermAndConditionService.GetAllTermsAndCondition($scope.OperationZone.OperationZoneId).then(function (response) {
