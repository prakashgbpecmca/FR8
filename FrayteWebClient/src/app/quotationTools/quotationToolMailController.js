angular.module('ngApp.quotationTools')
        .controller("QuotationToolMailController", function ($scope, quotationDetail, $rootScope, AppSpinner, toaster, QuotationService, $uibModalInstance, CustomerName, CustomerEmail, item, CustomerDetail, CustId, CustName, CustEmail, Header, MailContentText, SalesRepresentativeDetail, SessionService) {
            ...
            var setMultilingualOptions = function () {
                ...
                'ErrorSavingRecord', 'FrayteError', 'FrayteSuccess', 'SuccessfullySentMail', 'SendingMailError_Validation', 'Select_ServiceFirst']).then(function (translations) {
            ...
            $scope.sendQuotationMail = function (IsValid, EmailDetail) {
                ...
                if (IsValid) {
                    ...
                    if ($scope.quotationDetail !== undefined && $scope.quotationDetail.QuotationRateCard !== null && $scope.quotationDetail.QuotationRateCard.LogisticServiceId > 0) {
                        ...
                        AppSpinner.showSpinnerTemplate("Sending Quotation Email", $scope.Template);
                    ...
                    else if ($scope.item !== undefined && $scope.item !== null) {
                        AppSpinner.showSpinnerTemplate("Sending Quotation Email", $scope.Template);
                        var CustomerEmailDetail = {
                            QuotationShipmentId: item.QuotationShipmentId,
                            CustomerName: item.CustomerName,
                            CustomerEmail: item.CustomerEmail,
                            CCEmail: EmailDetail.ToCCEmail,
                            FromCountry: item.QuotationFromAddress.Country.Name,
                            ToCountry: item.QuotationToAddress.Country.Name,
                            FromPostCode: item.QuotationFromAddress.PostCode,
                            ToPostCode: item.QuotationToAddress.PostCode,
                            SenderName: SalesRepresentativeDetail.Name,
                            SenderEMail: SalesRepresentativeDetail.Email,
                            BCCEmail: EmailDetail.ToBCCEmail,
                            EMailText: EmailDetail.MailContent
                    ...
                    else if (CustomerDetail !== undefined && CustomerDetail !== null) {
                        AppSpinner.showSpinnerTemplate("Sending Rate Card Email", $scope.Template);
            ...
            function init() {
                ...
                {
                    $scope.Websitefrayte = 'www.frayte.com';
                }
                else {
                    $scope.Websitefrayte = 'www.frayte.co.uk';
                }
