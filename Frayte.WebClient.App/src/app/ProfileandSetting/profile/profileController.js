...
angular.module('ngApp.userprofile').controller('UserProfileController', function ($scope, ShipmentService, $rootScope, $state, $location, $translate, $filter, LogonService, SessionService, $uibModal, $log, uiGridConstants, toaster, Upload, config, CustomerService) {
    $scope.checkPasswordValidation = function (str) {
        if (str) {
            if (str.search(/[A-Z]/) < 0) {
                $scope.message = "Password must contain at least one capital letter.";
                $scope.error = true;
            }
            else if (str.search(/[0-9]/) < 0) {
                $scope.message = "Password must contain at least one digit.";
                $scope.error = true;
            }
    ...
    var setModalOptions = function () {
        $translate(['FrayteSuccess', 'Frayte-Error', 'SuccessfullyChangedPassword', 'FrayteWarning', 'FrayteValidation', 'Please_Correct_Validation_Error_First', 'InternalServerErrorOccurred', 'UploadedSuccessfully', 'ErrorOccuredDuringUpload', 'PleaseSelectValidImagefile']).then(function (translations) {
