angular.module('ngApp.zonePostCode').controller('ZonePostCodeAddEditController', function (AppSpinner, $scope, OperationZone, ZonePostCodeService, CourierCompany, ModuleType, ZonePostCode, $state, $location, $filter, $translate, SessionService, $uibModal, $log, uiGridConstants, toaster, ModalService, $uibModalInstance) {
    ...
    var setModalOptions = function () {
        $translate(['FrayteError', 'FrayteInformation', 'FrayteValidation', 'ErrorSavingRecord', 'PleaseCorrectValidationErrors', 'FrayteSuccess',
            ...
            'Successfully_Saved_Zone_PostCode', 'LoadingZonePostZipCode']).then(function (translations) {
                ...
                $scope.Successfully_Saved_Zone_PostCodes = translations.Successfully_Saved_Zone_PostCode;
    ...
    $scope.submit = function () {
        if ($scope.PostCodeJson === undefined || $scope.PostCodeJson === null) {
            $scope.PostCodeJson = {
                LogisticServiceZoneId: $scope.ZonePostCode.Row[0].PostCodeZone.ZoneId,
                TransitTime: $scope.TransitTime,
                FrayteZonePostCodeList: {}
            };
        }

        ZonePostCodeService.SavePostCode($scope.PostCodeJson).then(function (response) {
            AppSpinner.hideSpinnerTemplate();
            toaster.pop({
                type: 'success',
                title: $scope.Success,
                body: $scope.Successfully_Saved_Zone_PostCodes,
                showCloseButton: true
            });
        ...
        if (UkPost !== undefined && UkPost !== null) {
            ...
            if ($scope.PostCodeSaveJson.length > 0) {
                ...
                if (!flag) {
                    ...
                    $scope.PostCodeJson.FrayteZonePostCodeList = $scope.PostCodeSaveJson;
                }
            }
            else {
                $scope.PostCodeSaveJson.push(data);
    ...
    var getAllUKPostCode = function () {
        ...
        ZonePostCodeService.GetCountryUKPostCode($scope.BusinessOperationZone.OperationZoneId, "UKShipment", CourierCompany, RateType, ModuleType, $scope.ZonePostCode.Row[0].PostCodeZone.ZoneId).then(function (response) {
            AppSpinner.hideSpinnerTemplate();
            $scope.UKPostCodes = response.data.FrayteZonePostCodeList;
            $scope.UKPostCodeJson = response.data.FrayteZonePostCodeList;
            $scope.TransitTime = response.data.TransitTime;
