angular.module('ngApp.eCommerce').controller('uploadeCommerceBookingController', function ($uibModalStack, PostCodeService, $uibModalInstance, UtilityService, HSCodeService, $window, eCommerceBookingService, Upload, $rootScope, AppSpinner, $sce, TopCountryService, $location, $anchorScroll, TopCurrencyService, $scope, config, $filter, $state, ModalService, $translate, $uibModal, $timeout, toaster, $stateParams, DirectBookingService, SessionService, ShipmentId, UploadShipmentService, TrackingNo, CourierCompany) {
    ...
    $scope.PlaceBooking = function (DirectBookingForm, isValid, eCommerceBooking) {
        ...
        if ($scope.BookingType !== undefined && $scope.BookingType !== null && $scope.BookingType !== '') {
            ...
            if ($scope.BookingType === "Draft" || (isValid && $scope.isCollectionTimeValid)) {
                ...
                UploadShipmentService.SaveeCommerceUploadBooking(eCommerceBooking).then(function (response) {
    ...
    var SetMultilingualOtions = function () {
        ...
        'MaxWeight70kGS', 'MaxWeight154KGS', 'UploadingShipments', 'SavingInfomation']).then(function (translations) {
            ...
            if (eCommerceShipmentId > 0) {
                ...
                eCommerceBookingService.GeteCommerceBookingDetailDraft(eCommerceShipmentId, CallingType).then(function (response) {
                    ...
                    for (i = 0; i < $scope.eCommerceBooking.Packages.length; i++) {
                        ...
                        if ($scope.eCommerceBooking.Packages[i].CartoonValue === 0) {
                            $scope.eCommerceBooking.Packages[i].CartoonValue = null;
                        }
                        if ($scope.eCommerceBooking.Packages[i].Height === 0) {
                            $scope.eCommerceBooking.Packages[i].Height = null;
                        }
                        if ($scope.eCommerceBooking.Packages[i].Length === 0) {
                            $scope.eCommerceBooking.Packages[i].Length = null;
                        }
                        if ($scope.eCommerceBooking.Packages[i].Width === 0) {
                            $scope.eCommerceBooking.Packages[i].Width = null;
                        }
                        if ($scope.eCommerceBooking.Packages[i].Weight === 0 || $scope.eCommerceBooking.Packages[i].Weight * $scope.eCommerceBooking.Packages[i].CartoonValue > 70 * $scope.eCommerceBooking.Packages[i].CartoonValue) {
                            $scope.eCommerceBooking.Packages[i].Weight = null;
                            //toaster.pop({
                            //    type: 'warning',
                            //    title: $scope.TitleFrayteWarning,
                            //    body: "Maximum weight should be 70 Kgs per piece",
                            //    showCloseButton: true
                            //});
                        }
                    }
