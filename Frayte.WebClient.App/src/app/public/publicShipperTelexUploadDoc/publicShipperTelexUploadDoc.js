...
angular.module('ngApp.public').controller('PublicShipperTelexUploadController', function ($scope, $state, $stateParams, $translate, $location, config, $filter, SessionService, ShipmentService, Upload, $timeout, toaster) {
    ...
    var setModalOptions = function () {        
        $translate(['FrayteSuccess', 'FrayteError', 'FrayteValidation', 'ErrorGetting', 'PleaseCorrectValidationErrors', 'PleaseSelectValidFile',
            'ErrorOccuredDuringUpload', 'UploadedSuccessfully', 'PleaseAttachDocument', 'agent', 'records', 'ConfirmTelexRelease']).then(function (translations) {
            $scope.TitleFrayteError = translations.FrayteError;
            $scope.TitleFrayteValidation = translations.FrayteValidation;
            $scope.TitleFrayteSuccess = translations.FrayteSuccess;
            $scope.ConfirmTelexRelease = translations.ConfirmTelexRelease;
        ...
        if (isValid) {
            if (!$scope.confirmTelex) {
                toaster.pop({
                    type: 'warning',
                    title: $scope.TitleFrayteValidation,
                    body: $scope.ConfirmTelexRelease,
                    showCloseButton: true
                });
                return;
            }
