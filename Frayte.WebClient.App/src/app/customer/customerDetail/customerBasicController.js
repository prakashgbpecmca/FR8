    ...
    var getUserTab = function (tabs, tabKey) {
        if (tabs !== undefined && tabs !== null && tabs.length) {
            ...
            for (var j = 0 ; j < tab.childTabs.length; j++) {
                ...
                if ($scope.CustomerDetail.CustomerRateCardType === "NORMAL" && tab.childTabs[j].tabKey === "Advance_Rate_Card") {
                    tab.childTabs.splice(j, 1);
                }
                else if ($scope.CustomerDetail.CustomerRateCardType === "ADVANCE" && tab.childTabs[j].tabKey === "Customer_MarginCost") {
                    tab.childTabs.splice(j, 1);
                }
                else {
                    if (tab.childTabs[j].tabKey === "Advance_Rate_Card" && ($scope.customerId === 0 || $scope.customerId === "0")) {
                        tab.childTabs.splice(j, 1);
                    }
                }
            }
    ...
    function init() {
        ...
        CustomerService.GetCustomerDetail($scope.customerId).then(function (response) {
            ...
            if ($scope.CustomerDetail) {
                ...
                if ($scope.tab.childTabs && $scope.tab.childTabs.length) {
                    for (var i = 0 ; i < $scope.tab.childTabs.length; i++) {
                        if ($scope.tab.childTabs[i].tabKey === "Customer_MarginCost" && !$scope.CustomerDetail.IsRateCardAssigned) {
                            $scope.tab.childTabs[i].IsTabHide = true;
                        }
                        if ($scope.tab.childTabs[i].tabKey === "Advance_Rate_Card" && !$scope.CustomerDetail.IsRateCardAssigned) {
                            $scope.tab.childTabs[i].IsTabHide = true;
                        }
                        //if ($scope.tab.childTabs[i].tabKey === "Tracking_Configuration" && !$scope.CustomerDetail.IsTradeLaneBooking) {
                        //    $scope.tab.childTabs[i].IsTabHide = true;
                        //}
