...
.controller("CustomManifestDetailController", function ($scope, uiGridConstants,manifestObj, manifestId, $http, config, $window, $uibModal, toaster, SessionService, TermAndConditionService, ZoneBaseRateCardService, CustomerService, $rootScope, AppSpinner, $translate) {

    var setModalOptions = function () {
        $translate(['Frayte_Error', 'Removed_shipment_error', 'Removed_shipment', 'Frayte_Success', 'Error_While_Getting_Record', 'Downloaded_Status', 'Report_Generated_Successfully', 'Report_Status',
            'Could_Not_Download_TheReport', 'No_Report_Generated', 'LoadingManifestDetail', 'DownloadingDirectBookingManifestPDF']).then(function (translations) {          
    ...
    var getScreeninitials = function () {
        ...
        CustomerService.ViewCustomManifestDetail($scope.manifestId).then(function (response) {
            if (response.data !== null) {
                ...
                for (var i = 0; i < $scope.ViewManifest.length; i++) {
                    $scope.ViewManifest[i].collapseToggle = false;
                    $scope.ViewManifest[i].IsSelected = false;
                    $scope.ViewManifest[i].collapse = true;
                }
            }
            else {
