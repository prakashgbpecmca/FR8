...
.controller("CustomerManifestDetailController", function ($scope, uiGridConstants, manifestObj, manifestId, $http, config, $window, $uibModal, toaster, SessionService, TermAndConditionService, ZoneBaseRateCardService, CustomerService, $rootScope, AppSpinner, $translate) {
    var setModalOptions = function () {
        $translate(['Frayte_Error', 'Removed_shipment_error', 'Removed_shipment', 'Frayte_Success', 'Error_While_Getting_Record', 'Downloaded_Status',
            'Report_Generated_Successfully', 'Report_Status', 'Could_Not_Download_TheReport', 'No_Report_Generated', 'DownloadingDirectBookingManifestPDF', 'LoadingManifestDetail']).then(function (translations) {
            ...
            $scope.LoadingManifestDetail = translations.LoadingManifestDetail;
    ...
    $scope.TrackingPage = function (row) {
        ...
        if(row.DisplayName === 'UK Mail' || row.DisplayName === 'Yodel' || row.DisplayName === 'Hermes'){
            row.CourierName = 'UKEUShipment';
        }
        else {
            row.CourierName = row.ShippingBy;
        }        
    ...
    var getScreeninitials = function () {
        AppSpinner.showSpinnerTemplate($scope.LoadingManifestDetail, $scope.Template);
        // CustomerService.ViewCustomManifestDetail($scope.manifestId).then(function (response) {
        CustomerService.ViewManifest($scope.manifestId, $scope.moduleType, $scope.subModuleType).then(function (response) {
    ...
    $scope.DownloadmanifestReport = function () {
        if ($scope.manifestId !== undefined && $scope.manifestId !== 0) {
            ...
            CustomerService.GenerateManifest($scope.manifestId, $scope.ManifestDetail.ModuleType, $scope.UserId, $scope.RoleId).then(function (response) {
                AppSpinner.hideSpinner();
                var fileInfo = response.data;
                var fileName = {
                    FileName: response.data.FileName,
                    ModuleType: $scope.ManifestDetail.ModuleType
                };
                if (response.data != null) {
                    $http({
                        method: 'POST',
    ...
    function init() {
        ...
        $scope.moduleType = manifestObj.ModuleType;
        $scope.subModuleType = manifestObj.SubModuleType;
