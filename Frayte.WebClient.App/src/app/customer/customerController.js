angular.module('ngApp.customer').controller('CustomerController', function ($rootScope, $scope, UtilityService, $state, Upload, config, $location, $filter, $translate, CustomerService, ShipmentService, SessionService, $uibModal, uiGridConstants, toaster, $window, AppSpinner, ModalService) {
    ...
    var setModalOptions = function () {
        $translate(['Address', 'Book', 'DeleteHeader', 'DeleteBody', 'Tradelane', 'SuccessfullySavedInformation', 'FrayteInformation', 'FrayteValidation',
                    ...
                    'Shipment_Type', 'Select_CourierAccount', 'records', 'CustomerDelete_Validation', 'CustomerDeleteError_Validation',
                    'Delete', 'Confirmation', 'ACustomer_Record', 'Record_Deleted_Successfully', 'LoadingCustomers']).then(function (translations) {
                        ...
                        $scope.LoadingCustomers = translations.LoadingCustomers;
    ...
    $scope.AddEditCustomer = function (row) {  
        if ($scope.tab !== undefined && $scope.tab !== null && $scope.tab.childTabs !== null) {
            var route = UtilityService.GetCurrentRoute($scope.tab.childTabs, "userTabs.customer-detail.basic-detail");
            if (row === undefined) {
    ...
    $scope.SetCustomerGridOptions = function () {
        $scope.gridOptions = {
            ...
            columnDefs: [
              { name: 'CustomerAccountNo', displayName: "Frayte_Account_MEX", headerCellFilter: 'translate', width: '13%' },
              ...
              { name: 'UserAddress.Country.Name', displayName: "Country", headerCellFilter: 'translate', width: '13%' },
              { name: 'Email', headerCellFilter: 'translate', width: '21%' },
              { name: 'TelephoneNo', displayName: "Phone_No", headerCellFilter: 'translate', width: '12%' },
    ...
    $scope.LoadCustomers = function () {
        AppSpinner.showSpinnerTemplate($scope.LoadingCustomers, $scope.Template);
        ...
        CustomerService.GetCustomerList($scope.UserId).then(function (response) {
            ...
            for (i = 0; i < response.data.length; i++) {
                var newdata = "";
                if (UIdata[i].CustomerAccountNo !== null && UIdata[i].CustomerAccountNo !== undefined) {
            ...
            //Get Special Customer Information
            CustomerService.CustomerCompanyDetail($scope.UserId).then(function (response) {
                if (response.data === true) {
                    $scope.IsSpecialCustomer = true;
                }
                else {
                    $scope.IsSpecialCustomer = false;
                }
            });
        ...
        ShipmentService.GetInitials().then(function (response) {
            ...
            for (k = 0; k < data.length; k++) {
                for (l = 0; l < response.data.CountryPhoneCodes.length ; l++) {
                    if (data[k].UserAddress.Country !== null && data[k].UserAddress.Country !== undefined && data[k].UserAddress.Country.Code !== null && data[k].UserAddress.Country.Code !== undefined && data[k].UserAddress.Country.Code === response.data.CountryPhoneCodes[l].CountryCode) {
    ...
    function init() {
        ...
        $scope.spinnerMessage = $scope.LoadingCustomers;
