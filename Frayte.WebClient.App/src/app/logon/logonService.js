...
angular.module('ngApp.logon').factory('LogonService', function ($http, config) {
    ...
    var logon = function (credentials) {
        return $http.post(config.SERVICE_URL + '/Account/LoginUserDetail', credentials);
    };

    var changePassword = function (changePasswordDetail) {
        return $http.post(config.SERVICE_URL + '/Account/ChangePassword', changePasswordDetail);
    };
